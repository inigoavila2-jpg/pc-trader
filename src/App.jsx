...
function ToastContainer({toasts}) {
  const style=t=>({
    success:{accent:"#22c55e",icon:"✓",iconBg:"rgba(34,197,94,0.18)"},
    error:{accent:"#ef4444",icon:"✕",iconBg:"rgba(239,68,68,0.18)"},
    warn:{accent:"#f59e0b",icon:"!",iconBg:"rgba(245,158,11,0.18)"},
  }[t.type]||{accent:"#a78bfa",icon:"ℹ",iconBg:"rgba(167,139,250,0.18)"});
  return (
    <div style={{position:"fixed",top:"calc(14px + env(safe-area-inset-top))",right:14,left:14,zIndex:9999,
      display:"flex",flexDirection:"column",alignItems:"flex-end",gap:9,pointerEvents:"none"}}>
      {toasts.map(t=>{
        const s=style(t);
        return (
          <div key={t.id} style={{
            background:"#18181b",border:"1px solid #27272a",borderLeft:`3px solid ${s.accent}`,
            color:"#fff",padding:"11px 14px",borderRadius:12,fontSize:13,fontWeight:500,
            maxWidth:340,width:"fit-content",animation:"toastIn 0.35s cubic-bezier(0.22,1,0.36,1)",
            display:"flex",alignItems:"center",gap:10,
            boxShadow:"0 12px 32px rgba(0,0,0,0.5),0 2px 8px rgba(0,0,0,0.3)",overflow:"hidden",position:"relative"
          }}>
            <span style={{width:22,height:22,borderRadius:"50%",background:s.iconBg,color:s.accent,fontSize:12,fontWeight:800,
              display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{s.icon}</span>
            <span style={{lineHeight:1.4}}>{t.message}</span>
            <div style={{position:"absolute",bottom:0,left:0,height:2,background:s.accent,
              animation:"toastShrink 3.2s linear forwards",opacity:0.6}}/>
          </div>
        );
      })}
    </div>
...
function DefectiveModal({part,onConfirm,onCancel}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:1500,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onCancel}>
      <div style={{background:"#18181b",border:"1px solid #3f3f46",borderRadius:16,padding:22,width:"100%",maxWidth:380,boxShadow:"0 20px 60px rgba(0,0,0,0.6)",animation:"fadeUp 0.2s ease"}} onClick={e=>e.stopPropagation()}>
...
function Btn({children,variant="primary",onClick,disabled=false,loading=false,small=false,style={}}) {
  ...
  const VC={
    primary:{bg:"linear-gradient(135deg,#8b5cf6,#7c3aed)",hov:"linear-gradient(135deg,#7c3aed,#6d28d9)",txt:"#fff",bdr:"transparent"},
    ghost:{bg:"#27272a",hov:"#3f3f46",txt:"#d4d4d8",bdr:"#3f3f46"},
    danger:{bg:"rgba(127,29,29,0.5)",hov:"#991b1b",txt:"#fca5a5",bdr:"#ef4444"},
    success:{bg:"linear-gradient(135deg,#22c55e,#15803d)",hov:"linear-gradient(135deg,#16a34a,#166534)",txt:"#fff",bdr:"transparent"},
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",
      background:isDark?"radial-gradient(ellipse 1200px 800px at 50% -10%,#151518 0%,#09090b 55%)":bg,
      color:txt,fontFamily:"'Inter',system-ui,sans-serif",transition:"background 0.3s,color 0.3s"}}>
      <style>{`
          ...
          {ALL_TABS.map(t=>{
            ...
            return (
              <button key={t} onClick={()=>setTab(t)} style={{
                ...
                touchAction:"manipulation",WebkitTapHighlightColor:"transparent",
                transition:"opacity 0.15s ease",opacity:active?1:0.6,
              }}>
                <span style={{fontSize:16,lineHeight:1,filter:active?"none":"grayscale(0.4)",
                  padding:"5px 16px",borderRadius:99,
                  background:active?"rgba(124,58,237,0.18)":"transparent",
                  transition:"background 0.2s ease"}}>{TAB_ICONS[t]}</span>
