...
export function AIAgentChatbox({
  ...
  const SYSTEM_INSTRUCTION = `You are a helpful inventory assistant for a PC hardware reselling business.
...
When the user uploads a photo of hardware, read any visible labels, model names, and specs from the image and use them to write a good "name" for add_item_to_inventory. Combine that with whatever numbers (cost, market price) the user gives you. If you don't have the cost yet, ask for it — don't add the item until you know what it cost.

When the user wants to delete, undo, or return a sale ("delete the transaction with X", "undo this sale", "return item to inventory"), first call find_sales to locate the exact transaction. If you get more than one plausible match, ask the user which one they mean instead of guessing. Only after you have the exact saleId should you call delete_transaction.
